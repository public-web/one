...
restore.url = (args: { id: string | number } | [id: string | number] | string | number, options?: RouteQueryOptions) => {
    ...
    return restore.definition.url.replace('{id}', parsedArgs.id.toString()).replace(/\/+$/, '') + queryParams(options);
};
...
forceDelete.url = (args: { id: string | number } | [id: string | number] | string | number, options?: RouteQueryOptions) => {
    ...
    return forceDelete.definition.url.replace('{id}', parsedArgs.id.toString()).replace(/\/+$/, '') + queryParams(options);
};
...
const UserController = { index, store, update, destroy, restore, forceDelete };
